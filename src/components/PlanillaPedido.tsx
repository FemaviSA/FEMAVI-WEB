...
// No sabe de pases ni de permisos: la página le pasa de dónde salen los datos
// del cliente y con qué función de la base se guarda.
...
export function Casilla({ rot, span = 1, children }: { rot?: string; span?: number; children: React.ReactNode }) {
  return (
...
export interface FuentesDeCliente {
  /** Datos para autocompletar la planilla; null si no lo encuentra. */
  datos: (codigo: string) => Promise<DatosCliente | null>;
  /** Clientes cuyo nombre o CUIT se parece a lo escrito. */
  buscar: (q: string) => Promise<ClienteSugerido[]>;
}

interface Props {
  /** Las dos casillas del agente: fijas para el vendedor, un desplegable para el admin. */
  casillasAgente: React.ReactNode;
  /** De dónde salen los datos del cliente. Sin esto no hay autocompletado. */
  cliente?: FuentesDeCliente;
  /** Guarda el pedido con la función de la base que corresponda. */
  guardar: (input: OrderInput) => Promise<OrderCreated>;
  /** El pedido quedó guardado y el mail salió. */
  alGuardar: (pedido: OrderCreated) => void;
  /** Lo que la página exige además de lo de siempre (el admin, el vendedor). */
  validar?: () => string[];
  /** Devuelve true si el error ya lo resolvió la página y no hay que mostrarlo. */
  alFallar?: (e: unknown) => boolean;
  /** Se autocompletó con este cliente. */
...
export default function PlanillaPedido({
  ...
  useEffect(() => {
    ...
    if (!cliente || f.is_new_client || codigo.length < 2) {
  ...
  }, [f.client_code, f.is_new_client, cliente]);
  ...
  useEffect(() => {
    if (!cliente || f.is_new_client || nombreTipeado.trim().length < 3) {
      setSugerencias([]);
      return;
    }
    let vigente = true;
    const t = setTimeout(() => {
      cliente.buscar(nombreTipeado.trim())
        .then(r => { if (vigente) setSugerencias(r); })
        .catch(e => { if (vigente) alFallar?.(e); });
    }, 350);
    return () => { vigente = false; clearTimeout(t); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [nombreTipeado, f.is_new_client, cliente]);
  ...
  const submit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Si la página ya se ocupó (el pase del vendedor vencido, por ejemplo),
      // no hace falta mostrar nada más.
      if (alFallar?.(err)) return;
          ...
          <div style={{
            ...
            {datos?.nota && (
              <div style={{
                ...
                <b>Observación del sistema:</b> {datos.nota}
              </div>
            )}
